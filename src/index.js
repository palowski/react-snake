...
export default function Snake() {
  ...
  useEffect(() => {
    // pohlidam si udalost stisku klavesy
    document.addEventListener('keydown', handleChangeSnakeDirection); 
    // nastavim interval pohybu hada
              ...
              return <div style={{display: 'block', lineHeight: 0}}>{ 
                ...
                [...Array(GAME_SIZE.Width).keys()].map( x => {
                  ...
                  if (x === state.food.x && y === state.food.y)
                    return <div style={{ width:10, height:10, display: 'inline-block', backgroundColor: 'green', border: '1px solid #ccc'}}/> 
                  // POKUD se na vykreslovane pozici nachazi kterykoli kousek z tela hada
                  else if (state.snake.find( p => p.x === x && p.y === y) )
                    return <div style={{ width:10, height:10, display: 'inline-block', backgroundColor: '#aaa',  border: '1px solid #ccc'}}/> 
                  // JINAK je prazdne pole
                  else 
                    return <div style={{ width:10, height:10, display: 'inline-block', backgroundColor: '#eee',  border: '1px solid #ccc'}}/>})
              }</div>})} 
              { // pokud je had mrtev, zobraz tl.pro restart hry
                state.gameState === GAME_STATE.Killed && (<button onClick={ () => dispatch({type:ACTION.Start})}>killed! try again</button>)
              }
              </div>); 
}
